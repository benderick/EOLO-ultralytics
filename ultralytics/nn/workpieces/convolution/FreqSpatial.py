...
from ultralytics.nn.modules import C2f, Conv, CSP_T
from einops import rearrange

######################################## FreqSpatial start #######################################

__all__ = ["C2f_FreqSpatial"]

class ScharrConv(nn.Module):
    def __init__(self, channel, ph=None):
...
class FreqSpatial(nn.Module):
    style = 'i'
    def __init__(self, in_channels, ph=None):
...
class C2f_FreqSpatial(nn.Module):
    style = 'nij'
    def __init__(self, c1, c2, n=1, ph=None, e=0.5, g=1, shortcut=True):
        super().__init__()
        self.m = C2f_T(FreqSpatial, c1, c2, n, shortcut, g, e)
    def forward(self, x):
        return self.m(x)
# ----
if __name__ == "__main__":
    # Example usage
    model = FreqSpatial(in_channels=16)
    x = torch.randn(1, 16, 64, 64)  # Example input
    output = model(x)
    print(output.shape)  # Should print the shape of the output tensor
    model_csp = C2f_FreqSpatial(c1=16, c2=64, n=1)
    output_csp = model_csp(x)
    print(output_csp.shape)  # Should print the shape of the output tensor for CSP