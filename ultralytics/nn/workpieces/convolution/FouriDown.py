...
__all__ = ['FouriDown', 'BlurDown', 'CutDown']
...
class FouriDown(nn.Module):
    ...
    style = "ij"
    def __init__(self, in_channel, base_channel, *args):
...
class BlurDown(nn.Module):
    style = "ij"
    def __init__(self, in_channel, channels, ph1=None, ph2=None, pad_type='reflect', filt_size=4, stride=2, pad_off=0):
...
if __name__ == "__main__":
    # Example usage
    model = BlurDown(in_channel=3, channels=64, pad_type='reflect', filt_size=4, stride=2, pad_off=0)
    input_tensor = torch.randn(1, 3, 128, 128)  # Batch size of 1, 3 channels, 128x128 image
    output_tensor = model(input_tensor)
    print(output_tensor.shape)  # Should be (1, 64, 32, 32) for the given parameters
    m2 = CutDown(in_channel=64, base_channel=32)
    input_tensor2 = torch.randn(1, 64, 128, 128)
    output_tensor2 = m2(input_tensor2)
    print(output_tensor2.shape)  # Should be (1, 32, 64, 64) for the given parameters
    m3 = FouriDown(in_channel=64, base_channel=32)
    input_tensor3 = torch.randn(1, 64, 128, 128)
    output_tensor3 = m3(input_tensor3)
    print(output_tensor3.shape)  # Should be (1, 32, 64, 64) for the given parameters